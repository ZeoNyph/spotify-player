...
export default function PlaylistModal({ onClose, uri = "" }: PlaylistModalProps) {
    ...
    const [playlistInfo, setPlaylistInfo] = useState<PlaylistRequest | null>(null);
        ...
        <div className="fixed inset-0 flex flex-col gap-2 items-center justify-center bg-opacity-20 z-50 font-sans">
            <div className="bg-white rounded-lg shadow-lg p-6 min-w-[300px] max-h-[80dvh] overflow-y-auto">
                ...
                <div className="flex flex-col">
                    {playlistInfo && playlistInfo?.tracks?.items.length > 0 ? (
                        playlistInfo?.tracks?.items.map((item: Item) => (
                            <div key={item.track.id} className="py-2 flex flex-row items-center justify-between border-b last:border-b-0">
                                ...
                                <button onClick={async () => {
                                    ...
                                    await playPause({ isPlay: true, collectionURI: playlistInfo.uri, songURI: item.track.uri });
                                    onClose();
